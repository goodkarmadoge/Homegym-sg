...
test('smart + cable in a 1x2 room picks The Silent Operator', () => {
  assert.equal(
    nameOf({ functions: ['smart', 'cable'], length: 1, depth: 2, persona: 'Guided / Accountability User', budget: 6000 }),
    'The Silent Operator'
  );
});
...
test('barbell + cable on a $2,500 budget picks The Barbell Purist', () => {
  assert.equal(
    nameOf({ functions: ['power_rack', 'cable'], length: 2.5, depth: 3, persona: 'Practical / Value Seeker', budget: 2500 }),
    'The Barbell Purist'
  );
});
...
test('everything, 3x3, advanced, $7,000 picks The Iron Fortress', () => {
  assert.equal(
    nameOf({ functions: ['smith', 'power_rack', 'cable', 'leg_press'], length: 3, depth: 3, persona: 'Maximum Function User', budget: 7000 }),
    'The Iron Fortress'
  );
});
...
test('machine circuit, 1.5x2, beginner, $2,500 picks The Fast Track', () => {
  assert.equal(
    nameOf({ functions: ['multigym'], length: 1.5, depth: 2, persona: 'Convenience Seeker', budget: 2500 }),
    'The Fast Track'
  );
});
...
test('smith + cable, 1.5x2, intermediate, $4,000 picks The Foldaway Beast', () => {
  assert.equal(
    nameOf({ functions: ['smith', 'cable'], length: 1.5, depth: 2, persona: 'Convenience Seeker', budget: 4000 }),
    'The Foldaway Beast'
  );
});
...
test('orientation swap gives an identical result', () => {
  const base = { functions: ['smith', 'power_rack', 'cable'], persona: 'Serious Strength Trainer', budget: 5000 };
...
// Deliberately all or nothing. Personas are not on a scale the way beginner,
// intermediate and advanced were, so there is no honest partial credit for
// being "one persona away" from the customer in front of you.
test('personaScore is 1 for a bundle built for that persona and 0 otherwise', () => {
  assert.equal(personaScore('Convenience Seeker', ['Convenience Seeker']), 1);
  assert.equal(personaScore('Convenience Seeker', ['Serious Strength Trainer']), 0);
  // A bundle can serve several; bundle 3 serves two.
  assert.equal(personaScore('Maximum Function User', ['Maximum Function User', 'Serious Strength Trainer']), 1);
  // Name is typed by hand on two tabs, so comparison ignores case and spacing.
...
test('alternates never repeat the primary and never exceed two', () => {
  const r = run({ functions: ['smith', 'power_rack', 'cable'], length: 3, depth: 3, persona: 'Serious Strength Trainer', budget: 7500 });
...
test('the acceptance-criteria edge cases produce a result without throwing', () => {
  ...
  assert.doesNotThrow(() =>
    run({ functions: ['power_rack'], length: 1, depth: 1, persona: 'Convenience Seeker', budget: 2500 })
  );
  // $2,500 budget, 3x3 space, every function selected
  const all = run({
    functions: ['power_rack', 'smith', 'cable', 'leg_press', 'multigym', 'smart'],
    length: 3, depth: 3, persona: 'Maximum Function User', budget: 2500
  });
...
test('degenerate input does not throw', () => {
  ...
  assert.doesNotThrow(() => match({ functions: [], length: 0, depth: 0, persona: 'nobody', budget: 0 }, BUNDLES));
});

test('the matcher does not mutate the bundle data it is given', () => {
  const snapshot = JSON.stringify(BUNDLES);
  run({ functions: ['cable'], length: 2, depth: 2, persona: 'Convenience Seeker', budget: 4000 });
